...
```{r}
head(iris) # head() displays the first 6 items of an dataframe 
```
...
```{r}
names(iris) # to see the column names to write them correctly. 

# specific the column by name 
...
# specify the column on by its position
Iris2 <-  iris[,1] 
...
# note the entire list of columns within the c() function 
# comes after the comma to specify that it’s a column. 
...
# note, if you have sequential row and/or column numbers you can use a colon instead of c() which saves you much typing. 

Iris2 <-  iris[1:4,1:2] 
# this returns exactly the same as the previous line
...
# Check which Species are NOT "setosa"
iris$Species != "setosa"
...
```{r}
Iris2 <- iris[iris$Sepal.Width < 2.5,]
Iris2 <- iris[iris$Sepal.Width > 4.0,]
Iris2 <- iris[iris$Sepal.Width == 4.0,]
...
# here I am use multiuple lines to make the code easier to understand 
...
NB – dplyr has a nice function where it will automtically remove the caps on all variables! Let's use this to save typing
