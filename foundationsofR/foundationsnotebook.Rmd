...
mydata <- read.csv("data/mydata.csv") # note that if you called your folder 'Data'
...
df <- read_csv("data/tidyversedf.csv")
...
You can think of:
`Mutate()` = “add a new variable based on what is already there”

It always:

  - keeps the original data
  - adds new variables at the end
  - can use existing variables in calculations
...
### `summarise()` – summarising data (duh!)
...
`group_by()` = “do the next step separately for each category”
...
> **Exercise 4.1**: do you rememember what A, B and C stand-for (i.e. which is anth, psych and sociology)? Use mutate to re-label these variables with the correct, and more informative names and then re-save the table. 
