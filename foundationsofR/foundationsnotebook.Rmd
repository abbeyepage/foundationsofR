...
```
Remember, hwoever than you will need to re-save the vector (ideally with a different object name to avoiding overwriting. 
```{r}
my.vector # does it still include the 2nd element? 
My.vector2 <- my.vector[-2]  # what about now? 
```
...
Many functions return NA when missing values are present. You can explicitly remove them using indexing. 

```{r}
Mean(vec_with_na) # not working 
my.vector2 <- c(1,2,3,NA,4,5)
my.vector2 <- my.vector2[!is.na(my.vector2)]
my.vector2
Mean(my.vector2) # working 
```
> **Exercise 3.6**:  Create a numeric vector containing at least two NA values. Compute the mean with and without removing missing values.
